...
class GeminiProvider(LLMProvider):
    ...
    def __init__(self):
        ...
        if project_name is None:
            raise ValueError(
                "GCP_PROJECT_NAME is not set in the environment variables. Make sure to copy the .env.template file to .env and fill in the GCP_PROJECT_NAME."
            )
    ...
    def query(self, model_id: str, query: Query) -> QueryResponse:
        ...
        if model_id in ["G", "H", "I"]:
            ...
            if student_email is None:
                raise ValueError(
                    "STUDENT_EMAIL is not set in the environment variables. Make sure to copy the .env.template file to .env and fill in the STUDENT_EMAIL."
                )
        ...
        for turn in query.turns:
            for role, content in turn.items():
                ...
                if role not in ["user", "assistant"]:
                    raise ValueError(
                        f"Invalid role in query: {role}. Role must be one of 'user' or 'assistant'."
                    )
